      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-card border-r border-border transition-all duration-300 flex flex-col`}>
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-500 font-bold flex-shrink-0">
              {user?.name?.charAt(0)?.toUpperCase() || 'B'}
            </div>
            {sidebarOpen && (
              <div className="min-w-0">
                <p className="font-medium truncate">{user?.name}</p>
                <p className="text-xs text-foreground/60 truncate">Brand Partner</p>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 overflow-auto">
        <header className="bg-card/50 border-b border-border px-6 py-4 sticky top-0 z-10 backdrop-blur-lg">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold">{BRAND_TABS.find(t => t.id === activeTab)?.label}</h1>
              <p className="text-foreground/60 text-sm">Brand Dashboard</p>
            </div>
            <div className="flex items-center gap-3">
              <button className="p-2 hover:bg-border/50 rounded-lg transition-colors relative">
                ...
                {unreadNotifications > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-accent rounded-full" />}
              </button>
            </div>
          </div>
        </header>
